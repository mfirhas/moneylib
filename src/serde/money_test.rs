...
use crate::{BaseMoney, Money, macros::dec};
...
fn test_minor_deserialize_i128() {
    ...
    let result = crate::serde::money::minor::deserialize::<USD, _>(d);
    assert!(result.is_ok());
...
// ---------------------------------------------------------------------------
// Default deserializer: EUR and IDR with serde_json (via visit_map)
// ---------------------------------------------------------------------------

#[test]
fn test_default_deserialize_eur_json() {
    let money: Money<EUR> = serde_json::from_str("1234.56").unwrap();
    assert_eq!(money.code(), "EUR");
}

#[test]
fn test_default_deserialize_idr_json() {
    let money: Money<IDR> = serde_json::from_str("5000").unwrap();
    assert_eq!(money.code(), "IDR");
}

#[test]
fn test_default_deserialize_cad_json() {
    let money: Money<CAD> = serde_json::from_str("99.99").unwrap();
    assert_eq!(money.code(), "CAD");
}
...
fn test_default_deserialize_multi_currency_struct_json() {
    ...
    let m: Multi = serde_json::from_str(r#"{"eur":100.5,"idr":5000,"cad":99.99}"#).unwrap();
...
fn test_default_deserialize_yaml_mapping_error() {
    ...
    let result = serde_yaml::from_str::<Money<USD>>(r#"unexpected_key: value"#);
    assert!(result.is_err());
}

#[test]
fn test_default_deserialize_eur_yaml_mapping_error() {
    let result = serde_yaml::from_str::<Money<EUR>>(r#"unexpected_key: value"#);
    assert!(result.is_err());
}
...
fn test_default_serialize_to_value_usd() {
    let money = Money::<USD>::from_decimal(dec!(1234.56));
    let val = serde_json::to_value(&money).unwrap();
...
fn test_comma_str_code_serialize_to_value_usd() {
    ...
    assert_eq!(val["amount"], "USD 1,234.56");
}
...
fn test_default_deserialize_eur_yaml() {
    let money: Money<EUR> = serde_yaml::from_str("1234.56").unwrap();
    assert_eq!(money.code(), "EUR");
    let money2: Money<EUR> = serde_yaml::from_str("-500").unwrap();
    assert_eq!(money2.code(), "EUR");
}

#[test]
fn test_default_deserialize_idr_yaml() {
    let money: Money<IDR> = serde_yaml::from_str("50000.5").unwrap();
    assert_eq!(money.code(), "IDR");
    let money2: Money<IDR> = serde_yaml::from_str("-1000").unwrap();
    assert_eq!(money2.code(), "IDR");
}

#[test]
fn test_default_deserialize_cad_yaml() {
    let money: Money<CAD> = serde_yaml::from_str("99.99").unwrap();
    assert_eq!(money.code(), "CAD");
    let money2: Money<CAD> = serde_yaml::from_str("-50").unwrap();
    assert_eq!(money2.code(), "CAD");
}
...
fn test_default_deserialize_json_object_number_key_usd() {
    ...
    let money: Money<USD> =
        serde_json::from_str(r#"{"$serde_json::private::Number":"1234.56"}"#).unwrap();
    assert_eq!(money.code(), "USD");
    assert_eq!(money.amount(), dec!(1234.56));
}
...
fn test_default_deserialize_json_object_wrong_key() {
    ...
    let result = serde_json::from_str::<Money<USD>>(r#"{"wrong_key":"value"}"#);
    assert!(result.is_err());
...
fn test_default_deserialize_json_object_invalid_decimal() {
    ...
    let result =
        serde_json::from_str::<Money<USD>>(r#"{"$serde_json::private::Number":"not_a_number"}"#);
    assert!(result.is_err());
...
// ---------------------------------------------------------------------------
// Custom deserializers for triggering visit_unit and expecting in option visitors
// ---------------------------------------------------------------------------

// A deserializer whose deserialize_option calls visit_unit directly,
// which exercises the visit_unit path (handles formats like TOML that use unit for null).
struct VisitUnitDeserializer;

impl<'de> ::serde::de::Deserializer<'de> for VisitUnitDeserializer {
...
// A deserializer whose deserialize_option calls visit_bool, triggering
// the default Visitor::visit_bool which calls expecting() for error message.
struct BoolOptionDeserializer;

impl<'de> ::serde::de::Deserializer<'de> for BoolOptionDeserializer {
    type Error = ::serde::de::value::Error;
...
// ---------------------------------------------------------------------------
// visit_unit: option visitors accept unit type from custom deserializers
// ---------------------------------------------------------------------------

#[test]
fn test_option_comma_str_code_visit_unit_direct() {
    let result =
        crate::serde::money::option_comma_str_code::deserialize::<USD, _>(VisitUnitDeserializer);
    assert!(result.unwrap().is_none());
}

#[test]
fn test_option_comma_str_symbol_visit_unit_direct() {
    let result =
        crate::serde::money::option_comma_str_symbol::deserialize::<USD, _>(VisitUnitDeserializer);
    assert!(result.unwrap().is_none());
}

#[test]
fn test_option_dot_str_code_visit_unit_direct() {
    let result =
        crate::serde::money::option_dot_str_code::deserialize::<EUR, _>(VisitUnitDeserializer);
    assert!(result.unwrap().is_none());
}

#[test]
fn test_option_dot_str_symbol_visit_unit_direct() {
    let result =
        crate::serde::money::option_dot_str_symbol::deserialize::<EUR, _>(VisitUnitDeserializer);
    assert!(result.unwrap().is_none());
}

#[test]
fn test_option_minor_visit_unit_direct() {
    let result = crate::serde::money::option_minor::deserialize::<USD, _>(VisitUnitDeserializer);
    assert!(result.unwrap().is_none());
}

// ---------------------------------------------------------------------------
// expecting: option visitors and minor visitor produce meaningful error for unexpected type
// ---------------------------------------------------------------------------

#[test]
fn test_option_comma_str_code_expecting() {
    let result =
        crate::serde::money::option_comma_str_code::deserialize::<USD, _>(BoolOptionDeserializer);
    assert!(result.is_err());
    let msg = result.unwrap_err().to_string();
    assert!(
        msg.contains("string") || msg.contains("null") || msg.contains("USD"),
        "unexpected error: {}",
        msg
    );
}
...
fn test_option_comma_str_symbol_expecting() {
    let result =
        crate::serde::money::option_comma_str_symbol::deserialize::<USD, _>(BoolOptionDeserializer);
    assert!(result.is_err());
    let msg = result.unwrap_err().to_string();
    assert!(
        msg.contains("string") || msg.contains("null") || msg.contains("$"),
        "unexpected error: {}",
        msg
    );
}
...
fn test_option_dot_str_code_expecting() {
    let result =
        crate::serde::money::option_dot_str_code::deserialize::<EUR, _>(BoolOptionDeserializer);
    assert!(result.is_err());
    let msg = result.unwrap_err().to_string();
    assert!(
        msg.contains("string") || msg.contains("null") || msg.contains("EUR"),
        "unexpected error: {}",
        msg
    );
}
...
fn test_option_dot_str_symbol_expecting() {
    let result =
        crate::serde::money::option_dot_str_symbol::deserialize::<EUR, _>(BoolOptionDeserializer);
    assert!(result.is_err());
    let msg = result.unwrap_err().to_string();
    assert!(
        msg.contains("string") || msg.contains("null") || msg.contains("€"),
...
fn test_minor_expecting() {
    use serde::de::IntoDeserializer;
    type E = serde::de::value::Error;
    // BoolDeserializer::deserialize_any calls visit_bool; minor::Visitor has no visit_bool
    // so the default impl calls expecting() to format the InvalidType error.
    let d: serde::de::value::BoolDeserializer<E> = true.into_deserializer();
    let result = crate::serde::money::minor::deserialize::<USD, _>(d);
    assert!(result.is_err());
    let msg = result.unwrap_err().to_string();
    assert!(
        msg.contains("integer") || msg.contains("minor"),
        "unexpected error: {}",
        msg
    );
}
