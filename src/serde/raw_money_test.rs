...
fn test_option_comma_str_code_visit_unit_direct() {
    let result = crate::serde::raw_money::option_comma_str_code::deserialize::<USD, _>(
        VisitUnitDeserializer,
    );
    assert!(result.unwrap().is_none());
}
...
fn test_option_comma_str_symbol_visit_unit_direct() {
    let result = crate::serde::raw_money::option_comma_str_symbol::deserialize::<USD, _>(
        VisitUnitDeserializer,
    );
    assert!(result.unwrap().is_none());
}
...
fn test_option_dot_str_code_visit_unit_direct() {
    let result =
        crate::serde::raw_money::option_dot_str_code::deserialize::<EUR, _>(VisitUnitDeserializer);
    assert!(result.unwrap().is_none());
}

#[test]
fn test_option_dot_str_symbol_visit_unit_direct() {
    let result = crate::serde::raw_money::option_dot_str_symbol::deserialize::<EUR, _>(
        VisitUnitDeserializer,
    );
    assert!(result.unwrap().is_none());
}
...
fn test_option_minor_visit_unit_direct() {
    let result =
        crate::serde::raw_money::option_minor::deserialize::<USD, _>(VisitUnitDeserializer);
    assert!(result.unwrap().is_none());
}
...
fn test_option_dot_str_code_expecting() {
    let result =
        crate::serde::raw_money::option_dot_str_code::deserialize::<EUR, _>(BoolOptionDeserializer);
    assert!(result.is_err());
    let msg = result.unwrap_err().to_string();
    assert!(
