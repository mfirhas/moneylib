...
fn test_raw_split_dist_negative_total_rounded() {
    ...
    for p in &parts {
        assert!(
            !p.is_positive(),
            "expected non-positive part when splitting negative amount, got {}",
            p
        );
    }
...
fn test_raw_allocation_negative_total_rounded() {
    ...
    for p in &parts {
        assert!(
            !p.is_positive(),
            "expected non-positive part when allocating negative amount, got {}",
            p
        );
    }
...
fn test_raw_allocate_by_ratios_zero_middle() {
    ...
    assert!(
        parts[1].is_zero(),
        "zero-ratio part at index 1 should be zero, got {}",
        parts[1]
    );
