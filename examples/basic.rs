...
use moneylib::{
    BaseMoney, BaseOps, Currency, Money, MoneyFormatter, MoneyParser, RoundingStrategy,
...
fn main() {
    ...
    let money_from_str = Money::<USD>::from_str_code("USD 1,234.56").unwrap();
    println!("Parsed from string: {}", money_from_str);

    // Supports different thousand separator formats
    let money_euro = Money::<EUR>::from_str_code("EUR 1.234,56").unwrap();
    println!("European format: {}", money_euro);

    // Works without thousand separators too
    let money_simple = Money::<GBP>::from_str_code("GBP 500.25").unwrap();
